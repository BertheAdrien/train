...
const IMAGE_GRIS = new Image();
IMAGE_GRIS.src = 'images/gris.png';
...
function image_of_case(type_de_case){
	switch(type_de_case){
		...
		case Type_de_case.Gris	                : return IMAGE_GRIS;
...
function case_of_button(type_de_case){
	switch(type_de_case){
		...
		case "bouton_train_1"	                : return Type_de_case.Locomotive;
    }
}
...
function update_plateau(plateau, x, y){

	plateau.cases[x][y] = Type_de_case.Eau;

}

function getDisabledButtonId() {
	const buttons = document.querySelectorAll('button');
...
function printMousePos(e) {
	...
	// Calculer la position relative du clic par rapport à l'image
	var rect = img.getBoundingClientRect();
	var cursorX  = e.clientX - rect.left;
	var cursorY = e.clientY - rect.top;

	cursorX = Math.trunc(cursorX / LARGEUR_CASE)
	cursorY = Math.trunc(cursorY / HAUTEUR_CASE)
	
	return[cursorX, cursorY]
}

function loco_ou_pas(type_de_case){
	switch(type_de_case){
		case "bouton_foret" 				    : return false;
		case "bouton_eau"					    : return false;
		case "bouton_rail_horizontal"		    : return false;
		case "bouton_rail_vertical"			    : return false;
		case "bouton_rail_droite_vers_haut"  	: return false;
		case "bouton_rail_haut_vers_droite"	    : return false;
		case "bouton_rail_droite_vers_bas"	    : return false;
		case "bouton_rail_bas_vers_droite"	    : return false;
		case "bouton_train_1"	                : return true;
    }
}
...
function tchou(){
	...
	var img = document.getElementById("simulateur");

	img.style.backgroundColor = 'gray';
	
	img.addEventListener("click", function (e) {
		let mousePos = printMousePos(e);
		let x = mousePos[0];
		let y = mousePos[1];
		const buttonId = getDisabledButtonId();
		console.log(buttonId)

		if(buttonId != null){
			const loco = loco_ou_pas(buttonId)
				if(loco == false){
					plateau.cases[x][y] = Type_de_case.Gris;
					dessine_plateau(contexte, plateau);
				}
				
			plateau.cases[x][y] = case_of_button(buttonId);
